...
#define MAX_READ_OFFSET		0x200
#define MAX_READ_LENGTH		0x20
#define MAX_WRITE_LENGTH	0x20
...
#define MAX_START_ADDR 9223372036854775807LL 
...
static int get_start_address(long long *size, long long *start_addr)
{
	...
	if (!found){
		fprintf(stderr, "\"factory\" partition not found\n");
		goto leave;
	}

	*size = strtoll(p_size, NULL, 16);
	if ((*size <= 0) || (*size > MAX_START_ADDR)){
		fprintf(stderr, "size is out of range\n");
		goto leave;
	}

	*start_addr = strtoull(p_start_addr, NULL, 16);
	if ((*start_addr < 0) || (*start_addr > MAX_START_ADDR)){
		fprintf(stderr, "start_addr is out of range\n");
		goto leave;
	}
...
int emmc_read_offset(char *side, const unsigned int offset, const unsigned int len)
{
	...
	if (get_start_address(&size, &start_addr) < 0) {
		fprintf(stderr, "Get \"factory\" partition information failed\n");
		goto leave;
	}
	...
	if ((offset_t + offset + len) >= size) {
		fprintf(stderr, "exceed than the maximum size: \
			%s offset(0x%x) + read_offset(0x%x) + read_len(0x%x) \
			 > 0x%llx\n", side, offset_t, offset, len, size);
		goto leave;
	}
	offset_t += offset;
	...
	printf("read %s setting", side);
	for (i = 0; i < len; i++)
	{
		if ((i & 7) == 0)
			printf("\n%s[%08X]=", side, offset_t + i);
		else if ((i & 3) == 0)
			printf(" ");
...
int emmc_write_offset(char *side, const unsigned int offset, char *value)
{
	...
	if (len > MAX_WRITE_LENGTH){
		fprintf(stderr, "Range of len is [1-%d]\n", MAX_WRITE_LENGTH);
		goto leave;
	}
	...
	memset(buf, 0, len);
	if(buf == NULL){
		printf("%s malloc failed. There is not enough memory\n", __FUNCTION__);
		goto leave;
	}
...
int main(int argc,char **argv)
{
	...
	case 'r':
		...
		if ((offset < 0) || ( offset > MAX_READ_OFFSET)){
			fprintf(stderr, "Range of offset is [0-%d]\n", MAX_READ_OFFSET);
			goto Fail;
		}
		...
		if (emmc_read_offset(argv[2], offset, len) < 0)
#endif
			goto Fail;
		break;
	case 'w':
