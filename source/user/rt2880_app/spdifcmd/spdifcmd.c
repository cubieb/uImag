...
static int spdif_txbuffer_mmap(u32 buf_size)
{
	int i;
	//buf_size = 1024*12;
	for(i=0; i<(MAX_SPDIF_PAGE+1); i++)
	{
		printf("UbufSize=%d\n", buf_size);

		if(buf_size >= SPDIF_MIN_PAGE_SIZE)
		{
			buf_size = 1024*12;
			shtxbuf[i] = mmap(0, buf_size, PROT_WRITE, MAP_SHARED, spdif_fd, i*buf_size);
			if(shtxbuf[i] == MAP_FAILED)
			{
				printf("spdifcmd: failed to mmap1..\n");
				exit(1);
			}
			memset(shtxbuf[i], 0, buf_size);
		}
		else
		{
			shtxbuf[i] = mmap(0, SPDIF_MIN_PAGE_SIZE, PROT_WRITE, MAP_SHARED, spdif_fd, i*SPDIF_MIN_PAGE_SIZE);
			if(shtxbuf[i] == MAP_FAILED)
			{
				printf("spdifcmd: failed to mmap2..\n");
				exit(1);
			}
			memset(shtxbuf[i], 0, SPDIF_MIN_PAGE_SIZE);
		}
	}
	return 0;
}

static int spdif_txbuffer_munmap(u32 buf_size)
{
	int i;
	//buf_size = 1024*12;
	for(i=0; i<(MAX_SPDIF_PAGE+1); i++)
	{
		if(buf_size >= SPDIF_MIN_PAGE_SIZE){
			buf_size = 1024*12;
			if (munmap(shtxbuf[i], buf_size)!=0)
				printf("spdifcmd: munmap spdif mmap failed\n");
		}else{
			if (munmap(shtxbuf[i], SPDIF_MIN_PAGE_SIZE)!=0)
				printf("spdifcmd: munmap spdif mmap failed\n");
		}
	}
...
int main(int argc, char *argv[])
{
	...
	case 0:
		...
		while(left > buf_size){
			if (pause_stat == 0){
				ioctl(spdif_fd, SPDIF_PCM_PUT_AUDIO, &index);
				memcpy(shtxbuf[index], (char*)fdm+pos, buf_size);
				i++;
				if(i==6)
					ioctl(spdif_fd, SPDIF_TX_PCM_ENABLE, 0);
				left -= buf_size;
		...
		if (pause_stat == 0){
			ioctl(spdif_fd, SPDIF_PCM_PUT_AUDIO, &index);
			txbuffer = shtxbuf[index];
			memcpy(txbuffer, (char*)fdm+pos, left);
		}
#else
		while(left > buf_size){
			if (pause_stat == 0){
				ioctl(spdif_fd, SPDIF_PCM_PUT_AUDIO, (char*)fdm+pos);
				i++;
				if(i==6)
					ioctl(spdif_fd, SPDIF_TX_PCM_ENABLE, 0);
		...
		if (pause_stat == 0){	
			memcpy(txbuffer, (char*)fdm+pos, left);
			ioctl(spdif_fd, SPDIF_PCM_PUT_AUDIO, txbuffer);
		}
#endif
...
#if defined(CONFIG_SPDIF_MMAP)
		spdif_txbuffer_munmap(buf_size);
#endif
		break;	
  	case 1:
		if(argc < 4)
		...
		if (strtoul(argv[3], NULL ,10) == 1)
		{
			...
			{ 
				...
				ioctl(spdif_fd, SPDIF_NB_LEN, (nb_len << 3)); // parsing nb_len, unit is "bit" 
			}
		...
		while(left > nb_len){
			if(pause_stat == 0){
				ioctl(spdif_fd, SPDIF_RAW_PUT_AUDIO, &index);
				memcpy(shtxbuf[index], (char*)fdm+pos, nb_len);
				left -= nb_len;
				pos += nb_len;
			}
		}
		...
		while(left > nb_len)
		{
			if (pause_stat == 0)
			{
				ioctl(spdif_fd, SPDIF_RAW_PUT_AUDIO, (char*)fdm+pos);
				left -= nb_len;
				pos += nb_len;
			}
		}
		...
		{	
			...
			ioctl(spdif_fd, SPDIF_RAW_PUT_AUDIO, txrawbuffer);
		}
#endif
		ioctl(spdif_fd, SPDIF_TX_DISABLE, 0);	
		munmap(fdm, spdif_stat.st_size);
