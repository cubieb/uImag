...
static int hotspot_cli_cmd_wnmreq(struct hotspot_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int i = 0, ret;
	char rsp[2048];
...
static int hotspot_cli_cmd_wnmreq2(struct hotspot_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int i = 0, ret;
	char rsp[2048];
...
static int hotspot_cli_cmd_qosmap(struct hotspot_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int i = 0, ret;
	char rsp[2048];
...
static struct hotspot_cli_cmd hs_cli_cmds[] = {
	...
	{"btmreq [peer_mac] [ess_IMM] [disassoc_timer] [session_url]", hotspot_cli_cmd_btmreq,
	...
	{"wnmreq [peer_mac] [server url]", hotspot_cli_cmd_wnmreq, "send wnm notify request for remediation"},
	{"wnmreq2 [peer_mac] [code] [delay] [reason url(option)]", hotspot_cli_cmd_wnmreq2, "send wnm notify request for deauth imminent"},
	{"qosmap [peer_mac] [dscp_exception(option)] [dscp_range]", hotspot_cli_cmd_qosmap, "send qos map configure"},
};
