					...
					int *opmode, int *drv_mode, int *debug_level, int *version, char *iface)
{
	...
	while ((c = getopt(argc, argv, "m:f:i:d:v:")) != -1) {
		switch (c) {
		case 'd':
			cvalue = optarg;
...
int main(int argc, char *argv[])
{
	...
	/* options processing */
	ret = process_options(argc, argv, filename, &opmode, &drv_mode, &debug_level, &version, &hs.iface[0]);
	...
	if (child_pid == 0) {	
		...
		ret = hotspot_init(&hs, &hs_event_ops, drv_mode, opmode, version);
