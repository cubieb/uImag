...
static int i2s_txbuffer_mmap(void)
{
	...
	total_size = page_size*MAX_I2S_PAGE;

	//printf("tsize=%d, total=%d\n", page_size, total_size);	

	shtxbuf[0] = mmap(0, total_size, PROT_WRITE, MAP_SHARED, i2s_fd, 0);

	if (shtxbuf[0] == MAP_FAILED)
	{
		printf("i2scmd:failed to tx mmap..\n");
		exit(1);
	}
	memset(shtxbuf[0], 0, total_size);
	//printf("shtxbuf[0]=0x%08x\n", shrxbuf[0]);

	for(i = 1; i < MAX_I2S_PAGE; i++)
	{
		shtxbuf[i] = shtxbuf[0]+ i*page_size;
		//printf("shtxbuf[%d]=0x%08x\n", i, shtxbuf[i]);
	}
...
static int i2s_rxbuffer_mmap(void)
{
	...
	//printf("rsize=%d, total=%d\n", page_size, total_size);	

	shrxbuf[0] = mmap(0, total_size, PROT_READ, MAP_SHARED, i2s_fd, 0);
	
	if (shrxbuf[0] == MAP_FAILED)
	{
		printf("i2scmd:failed to rx mmap..\n");
		exit(1);
	}
	...
	for(i = 1; i < MAX_I2S_PAGE; i++)
	{
		shrxbuf[i] = shrxbuf[0]+ i*page_size;
		//printf("shrxbuf[%d]=0x%08x\n", i, shrxbuf[i]);
	}
	return 0;
}
...
static int i2s_txbuffer_munmap(void)
{
	...
	if(munmap(shtxbuf[0], total_size)!=0)
	{
		printf("i2scmd : munmap i2s tx mmap faild\n");
	}
...
static int i2s_rxbuffer_munmap(void)
{
	...
	if(munmap(shrxbuf[0], total_size)!=0)
	{
		printf("i2scmd : munmap i2s rt mmap faild\n");
	}
...
int main(int argc, char *argv[])
{
  ...
  case 1:
#if defined(MIC_IN_PATH)
#if defined(CONFIG_RALINK_MT7628) || defined(CONFIG_ARCH_MT7623) /*(CONFIG_I2S_WORD_LEN)*/
                if(argc < 9)
#else
                if(argc < 7)
#endif
			...
			{	
	    		...
	    		while(1)
	    		{
	    			...
	    			if( n > 0)
	    			{
	    				...
	    				{
	    					unsigned short *pData = (unsigned short *)pBuf;
		    			...
		    			{
		    				unsigned short *pData = (unsigned short *)txbuffer;
...
printf("tsize=%d\n", I2S_PAGE_SIZE);
		...
		while((pos+I2S_PAGE_SIZE)<=i2s_stat.st_size)
		{
			...
			pBufRx = rxbuffer;
			ioctl(i2s_fd, I2S_GET_AUDIO, pBufRx);
			memcpy((char*)fdm_in+pos, pBufRx, I2S_PAGE_SIZE);
