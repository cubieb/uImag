...
)
{
    ...
    if (gInterfaceHandle < 0) {
     ...
     //   printf("mtk_nfc_sys_i2c_gpio_write, ePin: %d, highlow: %d\n", ePin, eHighLow);
		ret = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_WRITE, ((ePin << 8) | (eHighLow)));
        //ret = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_WRITE, &ioctl_arg);
        if (MTK_NFC_SUCCESS != ret)
        {
            printf("nfc device ioctl fail, gpio_write, ret: %d\n", ret);
        }
    }    
}
...
)
{
    ...
    if (gInterfaceHandle < 0) {
        ...
        //ioctl_arg.pin = ePin;
		ret = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_READ, (ePin << 8));
        //ret = ioctl(gInterfaceHandle, MTK_NFC_IOCTL_READ, &ioctl_arg);
        //if (MTK_NFC_SUCCESS != ret)
        //{
        //    printf("nfc device ioctl fail, gpio_read, ret: %d\n", ret);
        //}
    }
    
    return ret;//ioctl_arg.highlow;
}
#endif
...
)
{
   ...
   {
      LOGD("mtk_nfc_sys_ioctl_command : node,%d , ioctl fail: %d, %s\n", gInterfaceHandle, errno, strerror(errno));
   }
   else
   {
      LOGD("mtk_nfc_sys_ioctl_command : Success.");
   }
