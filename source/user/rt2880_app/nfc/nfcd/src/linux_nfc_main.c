...
{    
    ...
    {       
       ...
       {
           //if ((nfc_msg != NULL) && (nfc_msg->type == MTK_NFC_DEBUG_REQ))
           if ((nfc_msg != NULL) && (g_dbg_file == 0) )//ming-chih modify to enable debug log
           {
                g_dbg_file = 1;
               UINT32* pData = NULL;
               pData = (UINT32*)((UINT8*)nfc_msg + sizeof(MTK_NFC_MSG_T));

               if (pData != NULL)
               {
                   //LOGD("dbg,%d,%d\n", (*pData), g_enable_dbg);
                   if (((*pData) == 1) && (g_enable_dbg == 0))
                   {
                       #ifdef DEBUG_LOG
                   ...
                   else if (((*pData) == 0) && (g_enable_dbg == 1))
                   {
                       g_enable_dbg = 0;
                       LOGD("disable,dbg,%d,\n",g_enable_dbg);                         
                   }
...
//           else if (nfc_msg != NULL)           
           ...
           {
               LOGD("nfc_msg null,%d,%d\n", ret, g_ThreadExitNfcAdaptation);
           }
       }
       else if (ret == (-3))
       {
