    ...
    if ((STP_UART_FULL == gStpMode) && (-1 == gTtyFd)) {
        ...
        if (ioctl(gTtyFd, TIOCSETD, &ld) < 0) {
         ...
         if (ioctl(gTtyFd, HCIUARTSETPROTO, 0) < 0) {
                printf("Can't set HCIUARTSETPROTO\n");
                return -4;
        }
        return 0;
    }
...
int cmd_hdr_stp_close (P_STP_PARAMS_CONFIG pStpParamsConfig) {
    ...
    if ((STP_UART_FULL == gStpMode) && (0 <= gTtyFd)) {
        ...
        ld = N_TTY;
        if (ioctl(gTtyFd, TIOCSETD, &ld) < 0) {
            printf("Can't restore line discipline");
            return -2;
        }
...
int cmd_hdr_sch_patch (P_STP_PARAMS_CONFIG pStpParamsConfig)
{
    ...
    if (gWmtFd > 0)
    {
        ...
        if(!strcmp(chipName,"mt6630"))//apply to MT6630
        {
            ...
            }
            {
                ...
                if (NULL == pDir)
                {
                    printf("patch path cannot be opened");
                    iRet = -1;
                    return iRet;
                }
	...
	if((!strcmp(chipName,"mt6620")) || (!strcmp(chipName,"mt6628")) || 
			...
			(!strcmp(chipName,"mt7623")) || (!strcmp(chipName,"mt6571")))
        {
#else
	if(1)
	{
		...
		}else if(!strcmp(chipName, "mt7623")){
			strncpy(chipName, "ROMv2_lm", strlen("ROMv2_lm") + 1);
		}else {
			strncpy(chipName, "ROMv2_lm", strlen("ROMv2_lm") + 1);
		}
        	strcat (chipName, "_patch");
...
static int get_wmt_cfg (int chipId)
{
#define WMTCFGFILEPATH "/etc_ro/wmt/WMT_SOC.cfg"
#define OPENMODE "r"
...
//		chipId = query_chip_id(); wujun
	...
	if((0x6582 == chipId) || (0x6592 == chipId) || (0x6572 == chipId) || (0x6571 == chipId) || (0x8127 == chipId) || (0x8590 == chipId))
	{
...
//haman, POLLRDNORM was not supported?
    fds[0].events = POLLIN; /* wait read events */
    ++fd_num;
