...
void usage(char *cmd)
{
	...
	printf("  wifi3_nvram_show   - display 3rd ralink device values in nvram\n");
	...
	printf("  wifi3    - 3rd ralink device\n");
#ifdef CONFIG_DUAL_IMAGE
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2) {
		...
		else if (!strncmp(argv[1], "wifi3_nvram_show", 17))
			nvram_show(WIFI3_NVRAM);
#ifdef CONFIG_DUAL_IMAGE
		...
		if (!strncasecmp(argv[1], "gen", 4) ||
			...
			else if (!strncasecmp(argv[2], "wifi3", 6))
				gen_config(WIFI3_NVRAM);
#ifdef CONFIG_DUAL_IMAGE
		...
		if (!strncasecmp(argv[1], "renew", 6)) {
			if (!strncmp(argv[2], "2860", 5) ||
				...
				renew_nvram(WIFI3_NVRAM, argv[3]);
#ifdef CONFIG_DUAL_IMAGE
			else if (!strncasecmp(argv[2], "uboot", 6))
...
#endif
	} else if (mode == WIFI3_NVRAM) {
		system("mkdir -p /etc/Wireless/WIFI3");
		fp = fopen("/etc/Wireless/WIFI3/RT2870AP.dat", "w+");
	} else
		return 0;
	if (NULL == fp){
		printf(" fopen failed.\n");
...
//#define FPRINT_STR(x) {char const* ptr; ptr=nvram_bufget(mode, #x);\
	...
	if ((RT2860_NVRAM == mode) 
		...
		FPRINT_STR(SSID7);
		FPRINT_STR(SSID8);
#if defined (CONFIG_16MBSSID_MODE) || defined (CONFIG_CHIP_MT7615E)
		FPRINT_STR(SSID9);
		FPRINT_STR(SSID10);
		FPRINT_STR(SSID11);
		FPRINT_STR(SSID12);
		...
		{
			snprintf(tx_rate+strlen(tx_rate), 3, ";%s",
					nvram_bufget(mode, "TxRate"));
		}
		fprintf(fp, "TxRate=%s\n", tx_rate);
		...
		FPRINT_STR(Key1Type);
		FPRINT_STR(Key1Str);  //For Lagency STA driver WPS.
		FPRINT_STR(Key1Str1);
		FPRINT_STR(Key1Str2);
		FPRINT_STR(Key1Str3);
		FPRINT_STR(Key1Str4);
		...
		FPRINT_STR(Key2Type);
		FPRINT_STR(Key2Str);  //For Lagency STA driver WPS.
		FPRINT_STR(Key2Str1);
		FPRINT_STR(Key2Str2);
		FPRINT_STR(Key2Str3);
		FPRINT_STR(Key2Str4);
...
#endif
		FPRINT_STR(Key3Type);
		FPRINT_STR(Key3Str);  //For Lagency STA driver WPS.
		FPRINT_STR(Key3Str1);
		FPRINT_STR(Key3Str2);
		FPRINT_STR(Key3Str3);
		FPRINT_STR(Key3Str4);
...
#endif
		FPRINT_STR(Key4Type);
		FPRINT_STR(Key4Str);  //For Lagency STA driver WPS.
		FPRINT_STR(Key4Str1);
		FPRINT_STR(Key4Str2);
		FPRINT_STR(Key4Str3);
		FPRINT_STR(Key4Str4);
...
#endif
#endif
		FPRINT_NUM(MACRepeaterEn);
		FPRINT_NUM(MACRepeaterOuiMode);
		FPRINT_STR(WscConfMode);
		FPRINT_STR(WscConfStatus);
/*
		//WscConfStatus
