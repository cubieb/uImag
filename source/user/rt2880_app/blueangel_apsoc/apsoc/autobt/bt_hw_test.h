...
#define ERR(f, args...)   (printf("[BT][ERR]%s: " f, __FUNCTION__, ##args))
#define WAN(f, args...)   (printf("[BT][WAN]%s: " f, __FUNCTION__, ##args))

#if BT_HW_TEST_DEBUG
#define DBG(f, args...)   (printf("[BT][DBG]%s: " f, __FUNCTION__, ##args))
#define TRC(f)            (printf("[BT][TRC]%s: #%d", __FUNCTION__, __LINE__))
#else
#define DBG(...)        ((void)0)
#define TRC(f)          ((void)0)
#endif
