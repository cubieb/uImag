...
static int btut_get_mod_index(const char *mod_name);
...
static BTUT_CLI bt_cnf_cli_commands[] = {
    { "set_log_lv", btut_set_log_lv_handler,
      "= set log level (0~5) of bluetooth.so" },
...
static int btut_get_mod_index(const char *mod_name)
{
    ...
    for (i = 0; i < g_btut_cntx.mods_cnt; i++)
    {
        ...
        if (!strncmp(mod->cmd_key, mod_name, sizeof(mod->cmd_key)))
        {
            index = i;
        }
    }
...
static char **btut_edit_cpl_cb(void *cntx, const char *cmd, int pos)
{
    ...
    else
    {
        ...
        if(end == NULL){
            fprintf(stderr, "[%s]cmd only has 1 string. <-- Should not happen\n", __func__);
            return NULL;
        }
        cmd = end + 1;
        end = strchr(cmd, ' ');
