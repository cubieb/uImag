...
#define bt_prompt_trace(mod, fmt, ...)    OS_Logd("%s : " fmt, __FUNCTION__, ## __VA_ARGS__)

#define LOG_ERR(f, ...)       OS_Loge("%s: " f, __FUNCTION__, ##__VA_ARGS__)
#define LOG_WAN(f, ...)       OS_Logw("%s: " f, __FUNCTION__, ##__VA_ARGS__)
#define LOG_DBG(f, ...)       OS_Logd("%s: " f, __FUNCTION__, ##__VA_ARGS__)
...
#define BTDRV_VERSION "0.0.0.4" //code base.revision.branch.patches
#define bt_android_log(fmt, ...)    OS_Logd("%s : " fmt, __FUNCTION__, ## __VA_ARGS__)

void GetVersion(void)
{
        printf("BT_BTDRV version =%s\n", BTDRV_VERSION);
        bt_android_log("BT_BTDRV version = %s\n", BTDRV_VERSION);
}
        ...
        if(BT_WritePacket(reset_cmd, sizeof(reset_cmd)) < 0){
            ...
            while(retry--){
                ...
                if(nRead < 0){
                    ...
                    dumpBinary(evt, nRead);
                    if(evt[0] == 4 && evt[1] == 0xE && evt[4] == 0x3 && evt[5] == 0xC){
                        break;
                    }                    
                }
            }
...
int  mtk_sco_op(int cmd)
{
 int ret=0;
    if (cmd)
          ret =ioctl(commPort, FORCE_SCO_ISOC_ENABLE, 0);
    else
         ret = ioctl(commPort, FORCE_SCO_ISOC_DISABLE, 0);
