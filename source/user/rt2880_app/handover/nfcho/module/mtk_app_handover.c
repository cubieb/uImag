...
INT32 mtk_nfcho_handover_token_build(UINT8 TokenData[],UINT16 TokenLength, UINT8 OutData[],UINT32* pOutLength, UINT8 Radio, UINT8 Idx, UINT8 TotalNum)
{
    ...
    if ((Radio < MTK_NFCHO_RADIO_END) && ((TokenLength != 0) && (TokenData != NULL)) &&((pOutLength != NULL) && (OutData != NULL)))
    {
        ...
        mtk_nfcho_service_dbg_output("token_b,L,%d",(*pOutLength));
    ...
    {
        mtk_nfcho_service_dbg_output("token_b,err,%d,%d",Radio, TokenLength);      
        ret = -1;
    }
    return ret;
}
...
INT32 mtk_nfcho_snep_msg_send(UINT8 field, UINT32 length, UINT32 acceptableLength, UINT8 data[])
{
   ...
   // mtk_nfcho_service_dbg_output("snep_send,%d", MsgLength); 
        ...
        #endif
    if ((field == SNEP_RESPONSE_DATA) || (field == SNEP_REQUEST_GET) || (field == SNEP_REQUEST_PUT) || (field == SNEP_REQUEST_REJECT))
    {
        mtk_nfcho_service_p2p_send_data(MTK_NFC_LLCP_SERVICEROLE_CLIENT, Data, MsgLength); // client 
    }
    else
    {
        mtk_nfcho_service_p2p_send_data(MTK_NFC_LLCP_SERVICEROLE_SERVER, Data, MsgLength); // server
    }
    if (Data != NULL)
    {
        free(Data);
...
INT32 mtk_nfcho_snep_parse(UINT8* u1Data, UINT16 u2Length, UINT8 u1ConnType, UINT8* pReadStatus, UINT8 EnSnep)
{
        ...
        else
        {
            if (u1ConnType == MTK_NFC_LLCP_SERVICEROLE_SERVER)
            {
                if ((*pReadStatus) == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (SnepReadLength == SnepReqLength)
                        {
                        ...
                        else if (SnepReadLength >= SnepReqLength)
                        {
                            // Send SNEP_RESPONSE_REJECT 
                ...
                else
                {
                    if (u2Length >= SNEP_HEADER_LENGTH)
                    {
                        ReqVer = u1Data[0];
                        ReqCmd = u1Data[1];
                        SnepReqLength = *(UINT32*)&u1Data[2];
                        SnepReqLength = SWAP32(SnepReqLength);
                        ReadData = u2Length-SNEP_HEADER_LENGTH;
    
                        mtk_nfcho_service_dbg_output("Ver,0x%x,Cmd,0x%x,SnepReqL,%d,ReadData,%d", ReqVer , ReqCmd, SnepReqLength , ReadData);
                        
                        if ( ((ReqVer & 0xF0)>>4) != SNEP_VERSION_MAJOR)
                        {
                            mtk_nfcho_service_dbg_output("UNSUPPORTED_VERSION,%d", ReqVer);
                        ...
                        else
                        {
                            ...
                            {   
                                ...
                                mtk_nfcho_service_dbg_output("ReqAccept,0x%x", ReqAcceptableLength);
                                ...
                                if (u2Length > 10 )
                                {
                                     ...
                                     if (ret == 0)
                                     {                    
                                         mtk_nfcho_service_dbg_output("ReqAccept,%d,SnepWrite,%d", ReqAcceptableLength , SnepWriteLength);                
                                         ...
                                         else 
                                         {
                                             ...
                                             mtk_nfcho_service_dbg_output("RESPONSE_NOT_FOUND,1");
                                ...
                                {
                                    //SNEP_RESPONSE_NOT_FOUND
                                    mtk_nfcho_service_dbg_output("RESPONSE_NOT_FOUND,2");
                                    mtk_nfcho_snep_msg_send(SNEP_RESPONSE_NOT_FOUND, 0, 0, NULL);
                                    (*pReadStatus) = 1; 
                                }
                            }
                            ...
                            {
                                memset(SnepReadData, 0x00, DEFAULT_ACCEPTABLE_LENGTH);
                                
                                SnepReadLength = ReadData;
                                memcpy(SnepReadData, &u1Data[SNEP_HEADER_LENGTH], ReadData);
                                mtk_nfcho_service_dbg_output("SnepReqL,%d,ReadData,%d", SnepReqLength, ReadData);                
                                if (SnepReqLength > ReadData)
                                {
                                    // Send SNEP_RESPONSE_CONTINUE
                                    mtk_nfcho_service_dbg_output("RESPONSE_CONTINUE");                       
                                    (*pReadStatus) = 0;
                            ...
                            {
                                mtk_nfcho_service_dbg_output("BAD_REQUEST");         
            ...
            else if(u1ConnType == MTK_NFC_LLCP_SERVICEROLE_CLIENT)
            {
                if ((*pReadStatus)== 0)
                {
                    ...
                    memcpy(&SnepReadData[SnepReadLength], u1Data, ReadData);
                    SnepReadLength = SnepReadLength + ReadData;
                    mtk_nfcho_service_dbg_output("snepL,%d,reqL,%d",SnepReadLength, SnepReqLength);                          
                    if (SnepReadLength == SnepReqLength)
                    {                  
                        (*pReadStatus) = 1;
                        ret = mtk_nfcho_handover_nego_mgt(SnepReadData, SnepReadLength, NULL, NULL);
                    ...
                    mtk_nfcho_service_dbg_output("ReadStatus,%d,SnepL,%d,ReadData,%d",(*pReadStatus), SnepReadLength, ReadData);  
                }   
                else
                {
                    if (u2Length >= SNEP_HEADER_LENGTH)
                    {
                        ...
                        if ( ((ReqVer & 0xF0)>>4) != SNEP_VERSION_MAJOR)
                        {
                            mtk_nfcho_service_dbg_output("UNSUPPORTED_VERSION,0x%x", ReqVer);
                            mtk_nfcho_snep_msg_send(SNEP_RESPONSE_UNSUPPORTED_VERSION, 0, 0, NULL);            
                        }
                        else
                        {
                            if (ReqCmd == SNEP_RESPONSE_SUCCESS)
                            {
                                ...
                                if (ReadData != 0)
                                {
                                    ...
                                    {
                                        // Send SNEP_RESPONSE_CONTINUE
                                        mtk_nfcho_service_dbg_output("REQUEST_CONTINUE");                       
                                        (*pReadStatus) = 0;
    ...
    else if (EnSnep == 0)
    {
        ...
        if (ret == 0)
        {
            mtk_nfcho_service_p2p_send_data(u1ConnType, SnepWriteData, SnepWriteLength);
            (*pReadStatus) = 1;        
        }
        else
        {
...
INT32 mtk_nfcho_snep_recv(UINT8 mode)
{
    INT32 ret = 0;
     
    ret = mtk_nfcho_service_p2p_receive_data(mode);
...
INT32 mtk_nfcho_snep_client_get( UINT8 SnepData[], UINT32 Length)
{
    ...
    ret = mtk_nfcho_snep_recv(MTK_NFC_LLCP_SERVICEROLE_CLIENT);
