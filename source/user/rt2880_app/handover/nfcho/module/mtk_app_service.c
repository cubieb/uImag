...
UINT8 g_nfc_device = 0;  // 1: reader, 2:p2p LLCP,
UINT8 g_nfc_restart = 0; // Restart flag
UINT8 ReaderOpt = 0;                                // 1: read , 2:write , 3:format
MTK_NFC_HANDLER p2p_working_handle[TOTAL_SERVICE];  // From client RSP , Client's handle
MTK_NFC_HANDLER p2p_server_handle = 0;

UINT32 p2p_min_miu[TOTAL_SERVICE];   // From server RSP

UINT32 p2p_sending[TOTAL_SERVICE];
UINT8  p2p_mode[TOTAL_SERVICE];  //1: server mode, 2 :client mode

UINT32 p2p_other_length[TOTAL_SERVICE];
UINT8  u1SendOtherData[TOTAL_SERVICE][PAYLOAD_LENGTH];

UINT32 RecvLength[TOTAL_SERVICE];                  // Reader / P2P receive  buffer  length
UINT8  u1RecvData[TOTAL_SERVICE][PAYLOAD_LENGTH];  // Reader / P2P receive  buffer 

UINT8 doneReading[TOTAL_SERVICE];                   // SNEP Reading fragmentation packet flag, 0 : Try to Receive the next PDU , 1 :Receive Done , 2 : Receive SNEP format error 3 :Wait SNEP_REQUEST_CONTINUE 

#ifdef DEFAULT_SUPPORT_HO
UINT8 SerNameH[] = "urn:nfc:sn:handover";  // Default LLCP service name , Default Handover 
UINT8 u1SAP = 0x01;                        // Default LLCP  SAP, Default Handover 
#else
...
INT32 mtk_nfc_app_service_start(UINT8 Action)
{
    ...
    if((Action == MTK_NFC_STOP_HWTEST_ALSPOLL) || (Action == MTK_NFC_STOP_HWTEST_ALSPOLL))
    {
        g_nfc_opt = Action;
        return mtk_nfcho_service_nfc_op(1);  
...
INT32 mtk_nfc_app_service_enter(MTK_NFC_APP_FUNCTION_POINT* pFunList, UINT8 TestMode)
{
    ...
    if (pFunList != NULL)
    {
        ...
        if ((TestMode == 0) || (TestMode == MTK_NFC_START_HWTEST_ALSPOLL) || (TestMode  == MTK_NFC_STOP_HWTEST_ALSPOLL))
        {
            if ((TestMode == MTK_NFC_START_HWTEST_ALSPOLL) || (TestMode  == MTK_NFC_STOP_HWTEST_ALSPOLL))
            {
                mtk_nfcho_service_nfc_hwtest(TestMode);
            }
            else
            {
...
INT32 mtk_nfc_app_service_nfcmwdebug(UINT8 Enable)
{

    mtk_nfcho_service_nfc_nfcmwdebug(Enable);
    return 0;
...
INT32 mtk_nfcho_service_nfc_hwtest(UINT8 TestMode)
{
    ...
    else if (TestMode == MTK_NFC_START_HWTEST_ALSPOLL)
    {
        UINT32  Req;
        
        Length = sizeof(UINT32);
        memset(&Req, 0x00, Length); 
        Req = NFC_EM_ACT_START;
        mtk_nfcho_service_dbg_output("nfc_test,poll,%d",Req); 
        mtk_nfcho_service_send_msg(MTK_NFC_FULLPOLL_REQ, (UINT8*)&Req, Length);  

    }
    else if (TestMode == MTK_NFC_STOP_HWTEST_ALSPOLL)
    {
        UINT32  Req;
...
INT32 mtk_nfcho_service_nfc_op(UINT8 Action) 
{
    ...
    if (Action == MTK_WIFINFC_SET_ON)
    {
        ...
        Req.p2p_setting.bm_mode = (P2P_MODE_PASSIVE);
    }    
    else if (Action == MTK_WIFINFC_SET_READER)
    {
        Req.tag_setting.autopolling = 1;
    ...
    else if (Action == MTK_WIFINFC_SET_CARD_READER_P2P)
    {
        ...
        Req.p2p_setting.status = 1;
        Req.p2p_setting.role = 2;
        Req.p2p_setting.bm_mode = (P2P_MODE_ACTIVE|P2P_MODE_PASSIVE);
    ...
    {
        for (idx = 0; idx < TOTAL_SERVICE ; idx++)
        {
            if (p2p_working_handle[idx] != 0)
            {
                mtk_nfcho_snep_stop(p2p_working_handle[idx]);
                p2p_working_handle[idx] = 0;
            }
        }
        Req.action = 0;
        g_nfc_device = 0;                   
    ...
    if (ret == 0)
    { 
        memset(p2p_working_handle, 0x00, (sizeof(MTK_NFC_HANDLER)*TOTAL_SERVICE));
        memset(p2p_min_miu, 0x00, (sizeof(UINT32)*TOTAL_SERVICE));
        memset(p2p_sending, 0x00, (sizeof(UINT32)*TOTAL_SERVICE));
        memset(p2p_mode, 0x00, (sizeof(UINT8)*TOTAL_SERVICE));
        memset(p2p_other_length, 0x00, (sizeof(UINT32)*TOTAL_SERVICE));
        memset(RecvLength, 0x00, (sizeof(UINT32)*TOTAL_SERVICE));
        memset(doneReading, 0x01, (sizeof(UINT8)*TOTAL_SERVICE));

        for (idx = 0; idx < TOTAL_SERVICE ; idx++)
        {
             memset(u1SendOtherData[idx], 0x00, PAYLOAD_LENGTH);
             memset(u1RecvData[idx], 0x00, PAYLOAD_LENGTH);             
        }
    ...
    switch(pMainMsg->type)
    {
         ...
         {             
             ...
             else if ((pPolling_Ntf->ntf.p2p.link_status == 0) && (pPolling_Ntf->detecttype == EM_ENABLE_FUNC_P2P_MODE))
             {
                 ...
                 mtk_nfcho_service_dbg_output("link,%d,%d", pPolling_Ntf->ntf.p2p.link_status, p2p_mode[idx]);          
                 ...
                 for (idx = 0; idx < TOTAL_SERVICE ; idx++)
                 {
                     if ( p2p_mode[idx] == MTK_NFC_LLCP_SERVICEROLE_CLIENT)
                     {
                        if (p2p_working_handle[idx] != 0)
                        {
                            mtk_nfcho_snep_stop(p2p_working_handle[idx]);
         ...
         case MTK_NFC_DISCOVERY_RSP:
         {
             ...
             if (pRsp != NULL)
             {
                 mtk_nfcho_service_dbg_output("%d,%d", pRsp->status , pRsp->result);             
                 if (pRsp->status == 1) 
                 {
                      if (pRsp->result == 0)
                      {
                          ...
                          if ((g_nfc_opt == MTK_NFC_STOP_HWTEST_ALSPOLL) || (g_nfc_opt == MTK_NFC_START_HWTEST_ALSPOLL))
                          {
                               mtk_nfcho_service_nfc_hwtest(g_nfc_opt);                          
                          }
                          else
                          {
                 ...
                 else
                 {
                      if (pRsp->result == 0)
                      {
                          mtk_nfcho_service_dbg_output("Stop,Poll,%d", g_nfc_opt);
                          if (g_nfc_restart == 1)
                          {
                              g_nfc_restart = 0;
                              mtk_nfcho_service_nfc_op(1);
                          }
                          else
                          {
                              g_nfc_device = 0;  
                      ...
                      {
                          mtk_nfcho_service_dbg_output("Stop,Poll,E,%d", g_nfc_opt);                       
                      }
                 }
             }
             else
         ...
         case MTK_NFC_P2P_LINK_NTF:
         {
             ...
             {                
                 ...
                 else if ((pRsp->ret == 0) && (pRsp->elink == MTK_NFC_LLCP_LINK_DEACTIVATE))
                 {
                     ...
                     mtk_nfcho_service_dbg_output("Bye Dev,%d", p2p_mode[idx]);                     
                     //Reset Variables
                     memset((UINT8*)RemoteRadioType, 0xFF, sizeof(UINT8)*MTK_NFCHO_RADIO_END);
                     g_nfc_device = 0;
         ...
         case MTK_NFC_JNI_TAG_READ_RSP:
         {
             ...
             RecvLength[0] = 0;
             memset(u1RecvData[0], 0x00, PAYLOAD_LENGTH);
             if (pRsp != NULL)
             {
                // For Debug
                //  mtk_nfcho_service_dbg_output("Read,Tag, (%d)=(%d)", pRsp->result  , pRsp->len );
                 ...
                 {
                     RecvLength[0]= pRsp->len;
                     memcpy(u1RecvData[0], &pRsp->data, RecvLength[0]);
                     ...
                     #endif 
                     reulst = mtk_nfcho_handover_static_mgt(u1RecvData[0], RecvLength[0]);
                     if (reulst == (-1))
                     {
                         Ret = 0xFF;
                     }
         ...
         case MTK_NFC_P2P_CREATE_SERVER_NTF:
         {
             ...
             if (pSerNtf->ret == 0)
             {
                 ...
                 p2p_working_handle[idx] = pSerNtf->incoming_handle;
                 if (pSerNtf->remote_miu < MTK_NFC_HO_P2P_MAX_DATA)
                 {
                      p2p_min_miu[idx] = pSerNtf->remote_miu; 
                 }
                 else
                 {
                      p2p_min_miu[idx] = MTK_NFC_HO_P2P_MAX_DATA;
                 }
                 p2p_mode[idx] = MTK_NFC_LLCP_SERVICEROLE_SERVER;
                 doneReading[idx] = 1;                 
                 mtk_nfcho_service_p2p_accpet_server(pSerNtf->incoming_handle);
             }          
             mtk_nfcho_service_dbg_output("SERVER_NTF,ret,%d,%d,WH,0x%x,SH,0x%x,%d,%d", pSerNtf->ret ,idx, p2p_working_handle[idx], pSerNtf->service_handle, p2p_mode[idx], p2p_min_miu[idx]);             
         }
         break;
         ...
         {            
             pBuf = ((UINT8*)pMainMsg+sizeof(MTK_NFC_MSG_T));
             mtk_nfcho_service_dbg_output("ACCEPT_RSP,%d", *(INT32*)pBuf);
             if ( *(INT32*)pBuf == 0)
             {
                 mtk_nfcho_service_p2p_receive_data(MTK_NFC_LLCP_SERVICEROLE_SERVER);
             }
         }
         break;
         ...
         case MTK_NFC_P2P_CREATE_CLIENT_RSP:
         {
             ...
             {
                 idx = 1;         
                 p2p_working_handle[idx] = pSerRsp->llcp_handle;
                 p2p_mode[idx] = MTK_NFC_LLCP_SERVICEROLE_CLIENT;
                 doneReading[idx] = 1;                 
                 mtk_nfcho_service_p2p_connect_client(pSerRsp->llcp_handle);
             } 
             mtk_nfcho_service_dbg_output("CLIENT_RSP,%d,SH,0x%x",pSerRsp->ret, p2p_working_handle[idx]);
         }
         break;
         ...
         case MTK_NFC_P2P_CONNECTION_NTF:
         {
             ...
             if (( pConnNtf->ret == 0) && (pConnNtf->errcode == 0))
             {
                 ...
                 if (pConnNtf->remote_miu < MTK_NFC_HO_P2P_MAX_DATA)
                 {
                     p2p_min_miu[idx] = pConnNtf->remote_miu; 
                 }
                 else
                 {
                      p2p_min_miu[idx] = MTK_NFC_HO_P2P_MAX_DATA;
             ...
             else if (( pConnNtf->ret == 1) && (pConnNtf->errcode == 0))
             {          
                 Ret = 0;
                 mtk_nfcho_service_event_handle(MTK_NFC_APP_EVENT_WRITE, 0, 0, 1, &Ret );                
             }
             mtk_nfcho_service_dbg_output("CONN_NTF,%d,%d,mode,%d,%d", pConnNtf->ret, pConnNtf->errcode ,p2p_mode[idx], p2p_min_miu[idx]);             
         }
         break;
         ...
         case MTK_NFC_P2P_SEND_DATA_RSP:
         {
             ...
             {          
                 for (idx = 0; idx < TOTAL_SERVICE; idx++)
                 {
                     if (p2p_working_handle[idx] == pRsp->service_handle)
                     {
                         break;
                     }
                 }
                 if (idx < TOTAL_SERVICE)
                 {           
                      mtk_nfcho_service_dbg_output("SEND_RSP,%d,%d,%d,%d", idx, p2p_mode[idx], p2p_sending[idx], p2p_other_length[idx]);
                     
                     if (doneReading[idx] == 2)
                     {   
                     ...
                     {
                         mtk_nfcho_service_dbg_output("Rec,pdu,%d", doneReading[idx]);
                         mtk_nfcho_service_p2p_receive_data(p2p_mode[idx]);
                     }
                 }                 
             }
             else if ( pRsp->ret != 0)
             {
                 ...
                 if (idx < TOTAL_SERVICE)
                 {
                     ...
                     if (p2p_sending[idx] == 1)
                     {
                         p2p_sending[idx] = 0;
                     }
                     if (p2p_working_handle[idx] != 0)                 
                     {
                         mtk_nfcho_snep_stop(p2p_working_handle[idx]); // Error case    
                     }
                 }
             }
         }
         ...
         case MTK_NFC_P2P_RECV_DATA_RSP:
         {
             ...
             if ( pSerRsp->ret == 0)
             {
                  for (idx = 0; idx < TOTAL_SERVICE; idx++)
                  {
                      if (p2p_working_handle[idx] == pSerRsp->service_handle)
                      {
                          break;
                      }
                  }
                  ...
                  if (idx < TOTAL_SERVICE)
                  {             
                      LocalRecvLth = pSerRsp->llcp_data_recv_buf.length;
                      memcpy((u1RecvData[idx] +RecvLength[idx]), pSerRsp->llcp_data_recv_buf.buffer, LocalRecvLth);
    
                    //  mtk_nfcho_service_dbg_output("RecvL,%d", RecvLength );
                      if (u1SAP == 0x04)
                      {
                          RecvLength[idx] = LocalRecvLth;
                          mtk_nfcho_snep_parse( u1RecvData[idx], (UINT16)RecvLength[idx], p2p_mode[idx], &doneReading[idx], 1); //With SNEP
                      }
                      else 
                      {
                          if (LocalRecvLth < p2p_min_miu[idx])
                          {
                              RecvLength[idx] = RecvLength[idx] + LocalRecvLth;
                              mtk_nfcho_snep_parse(u1RecvData[idx], (UINT16)RecvLength[idx], p2p_mode[idx], &doneReading[idx], 0); //Without SNEP
                          }
                          else if (LocalRecvLth == p2p_min_miu[idx])
                          {
                               RecvLength[idx] = RecvLength[idx] + LocalRecvLth;
                               if (RecvLength[idx] < PAYLOAD_LENGTH)                           
                               {
                                   doneReading[idx] = 0;
                               }
                               else
                      ...
                      mtk_nfcho_service_dbg_output("Recv,%d,%d,%d,%d",idx,p2p_mode[idx], doneReading[idx],RecvLength[idx]);
    
                      if ( doneReading[idx] == 0)// Try to Receive the next PDU 
                      {
                         ret = mtk_nfcho_service_p2p_receive_data(p2p_mode[idx]);
                      }     
                      else if ( doneReading[idx] == 1) // Receive Done 
                      {     
                          if (p2p_mode[idx] == MTK_NFC_LLCP_SERVICEROLE_CLIENT) //Client mode Receive done
                          {
                              mtk_nfcho_service_dbg_output("ClRecvDone");
                              if ((p2p_sending[idx] == 1) && (p2p_other_length[idx] > 0))
                              {
                                  UINT8* pData = NULL;
                                  UINT16 u2DataLength = p2p_other_length[idx];
            
                                  pData = (UINT8*)malloc(u2DataLength);
                                  memset(pData, 0x00, u2DataLength);
                                  memcpy(pData, u1SendOtherData[1], u2DataLength);
                                  p2p_sending[idx] = 0;
                                  
                                  mtk_nfcho_service_p2p_send_data(p2p_mode[idx], pData, u2DataLength); 
                                  free(pData);
                                  pData = NULL;
                              }                                  
                              else if (p2p_working_handle[idx] != 0)                          
                              {
                                  mtk_nfcho_snep_stop(p2p_working_handle[idx]);    
                              }
                          }
                      }
                      else if ( doneReading[idx] == 2)// SNEP format Error 
                      {
                      ...
                      else if ( doneReading[idx] == 3) // Wait SNEP_REQUEST_CONTINUE 
                      {
                          mtk_nfcho_service_dbg_output("WaitContinue");
                      }
                  }
             }
             else if ( pSerRsp->ret != 0)
             {
                  ...
                  if (idx < TOTAL_SERVICE)
                  {
                     mtk_nfcho_service_dbg_output("Recv,err,%d,%d,%d,%d", pSerRsp->ret, idx, p2p_mode[idx] , doneReading[idx]);

                     if (doneReading[idx] == 0)
                     {
                         doneReading[idx] = 1;
                     }
                     if (p2p_mode[idx] == MTK_NFC_LLCP_SERVICEROLE_CLIENT)
         ...
         case MTK_NFC_FULLPOLL_RSP:
         {
             UINT32* pRspData = NULL;
             pRspData = (UINT32*)((UINT8*)pMainMsg + sizeof(MTK_NFC_MSG_T));

             if (pRspData != NULL)
             {
                 Result = (*pRspData);
                 mtk_nfcho_service_dbg_output("alspoll,%d", Result);                    
             }
             else
             {
                 mtk_nfcho_service_dbg_output("null,%d", pMainMsg->type);
             ...
             g_nfc_opt = 1;
             mtk_nfcho_service_nfc_op(g_nfc_opt);                 
         }
         break;
...
INT32 mtk_nfcho_service_p2p_create_server(UINT8 SerName[], UINT32 Length, UINT8 uSap)
{
    ...
    if (pSer != NULL)
    {
        ...
        pSer->buffer_options.miu = MTK_NFC_HO_P2P_MAX_DATA;
...
INT32 mtk_nfcho_service_p2p_accpet_server(UINT32 Handle)
{
  ...
  //  mtk_nfcho_service_dbg_output("accpet,0x%x", Handle );
    ...
    SerAccept.buffer_options.miu = MTK_NFC_HO_P2P_MAX_DATA;
...
INT32 mtk_nfcho_service_p2p_send_data(UINT8 mode, UINT8* DataBuf, UINT32 DataLth)
{
    ...
    if (idx < TOTAL_SERVICE)                  
    {         
        if (DataLth > p2p_min_miu[idx])
        {
            mtk_nfcho_service_dbg_output("send,L,%d,min,L,%d,%d,%d", DataLth,idx,mode, p2p_min_miu[idx]); 
            memset(u1SendOtherData[idx], 0x00, PAYLOAD_LENGTH);
            p2p_other_length[idx] = DataLth - p2p_min_miu[idx];
            SendDataLength = p2p_min_miu[idx];
            p2p_sending[idx] = 1; 
            memcpy(u1SendOtherData[idx], &DataBuf[p2p_min_miu[idx]], p2p_other_length[idx] );
        }  
        else
        {
        ...
        mtk_nfcho_service_dbg_output("send,H,0x%x,L,%d", p2p_working_handle[idx], SendDataLength);     
        ...
        if (pSendData != NULL)
        {
            ...
            pSendData->service_handle = p2p_working_handle[idx];
            pSendData->sap = 0x20;
...
INT32 mtk_nfcho_service_p2p_receive_data(UINT8 mode)
{
    ...
    if (idx < TOTAL_SERVICE)          
    {
        mtk_nfcho_service_dbg_output("recv,%d,%d,0x%x",idx ,mode, p2p_working_handle[idx]); 
        if (p2p_working_handle[idx] != 0)
        {  
            if (doneReading[idx] != 0)
            {
            ...
            RecvReq.llcp_service_handle = p2p_working_handle[idx];        
...
INT32 mtk_nfcho_service_nfc_nfcmwdebug(UINT8 enable)
{
    UINT32 Req;
    UINT32 Length = sizeof(UINT32);
    INT32 ret = 0;
 
   // mtk_nfcho_service_dbg_output("dbg,%d", enable); 

    if ((enable == 1) || (enable == 0))
    {
        memset(&Req, 0x00, sizeof(UINT32));
        Req = enable;     
        mtk_nfcho_service_send_msg(MTK_NFC_DEBUG_REQ, (UINT8*)&Req, Length);        
    }
    return ret;  
}
...
INT32 mtk_nfcho_service_send_msg(UINT32 type, UINT8* payload, UINT32 len)
{
   ...
   mtk_nfcho_service_dbg_output("t,%d", type);
    ...
    if ((DataLth != NULL) && (DataBuf != NULL) && (NumRadio != 0))
    {
        ...
        {                       
            ...
            { 
                ...
                {
                    mtk_nfcho_service_p2p_send_data(MTK_NFC_LLCP_SERVICEROLE_CLIENT, OutData, OutLength);
          //          mtk_nfcho_service_p2p_receive_data(MTK_NFC_LLCP_SERVICEROLE_CLIENT);
                }
            }
            else
            {
        ...
        else if (IsRadio == 0)
        {
             ...
             {
                 mtk_nfcho_service_p2p_send_data(MTK_NFC_LLCP_SERVICEROLE_CLIENT, DataBuf[idx], DataLth[idx]);
           //      mtk_nfcho_service_p2p_receive_data(MTK_NFC_LLCP_SERVICEROLE_CLIENT);
             } 
        }
        else
