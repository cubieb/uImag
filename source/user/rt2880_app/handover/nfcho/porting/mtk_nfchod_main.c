...
int g_bt_pair = 0;  // 1: BT Pairing   0: WPS   /* if use WPS need to remark mtk_app_service.h DEFAULT_SUPPORT_HO */
...
//unsigned char SupportRadioType = (1 << MTK_NFCHO_RADIO_BT); //Deafult BT 
...
UINT8 g_BTPayload[8] = { 0x00 ,   //Payload Length
                        0x08 ,
                        0x01,//0x52 ,   //BT Mac addrfess
                        0x23,//0x21 ,
                        0x76,//0xa7 ,
                        0x46,//0x43 ,
                        0x00,//0x0c ,
...
{  
   ...
   {
      MTK_NFCAPP_LOGD("r,(t,%d)", msg_hdr.type);
...
{   /* exit thread by pthread_kill -> pthread_join*/
 ...
 //   void *ret;
    ...
    if(arg->thread_id == MTK_NFCHO_THREAD_SERVICE)
    {
		...
		printf("mtk_nfcho_exit_thread_normal: g_MtkNfchoServiceThreadExit = 1\n");
    }
...
int mtk_nfcho_threads_create(MTK_NFCHO_THREAD_ID_ENUM thread_id)
{
    ...
    if (thread_id == MTK_NFCHO_THREAD_SERVICE)
    {
	   ...
	   printf("mtk_nfcho_threads_create: g_MtkNfchoServiceThreadExit = 0\n");
       ...
       {
           g_MtkNfchoServiceThreadExit = 1;
		   printf("mtk_nfcho_threads_create: g_MtkNfchoServiceThreadExit = 1\n");
           ret = -1; 
       }
    }
...
int mtk_nfcho_init (void)
{
    ...
    {
        MTK_NFCAPP_LOGD("nfcho_init,pid,%d", g_pid);
    }
    usleep(500*1000);
    ...
    {
        MTK_NFCAPP_LOGE("socket_fd,fail");
    }    
    // Create threads
    ...
    if (NfcTsetMode == 1)
    {
        ...
        g_MtkNfchoServiceThreadExit = 0;
		printf("mtk_nfcho_init: g_MtkNfchoServiceThreadExit = 0\n");
    }
    else
    {
      ...
      switch (Event)
      {
            case MTK_NFC_APP_EVENT_RESULT:
            {
                if ((DataType == MTK_NFC_ENTER_RSP) && (DataLth == 1))
                {
                     ...
                     {
                         unsigned char Cmd = 0;
                         if (NfcOpCmd == MTK_WIFINFC_SET_START_FULL_POLL)
                         {
                              Cmd = MTK_NFC_START_HWTEST_ALSPOLL;
                         }
                         else if (NfcOpCmd == MTK_WIFINFC_SET_STOP_FULL_POLL)
                         {
                             Cmd = MTK_NFC_STOP_HWTEST_ALSPOLL;
                         }
                         else
                         {
                             Cmd = NfcOpCmd;
                         }
                         mtk_nfc_app_service_start(Cmd);                        
                     }
                ...
                {
                    MTK_NFCAPP_LOGE("result,%d,%d",DataType, DataLth);
                }
            }
            break;                   
           ...
           {  
               ...
               MTK_NFCAPP_LOGD("Read,%d,%d,%d,%d", DataType, CmdType, TypeLth, DLth );
           ...
           case MTK_NFC_APP_EVENT_FOUNDTAG:
           {
               ...
               if (Event == MTK_NFC_APP_EVENT_FOUNDDEVICE)
               {
                   ...
                   else
                   {
                       ...
                       {
                           MTK_NFCAPP_LOGD("Event send WiFi");
           ...
           case MTK_NFC_APP_EVENT_KILLDONE:
           {
               ...
               NfcStatus = 0;               
               g_MtkNfchoServiceThreadExit = 1;
			   printf("mtk_nfcho_nfc_event_handle: g_MtkNfchoServiceThreadExit = 1\n");
               g_MtkNfchoMainProcessExit = 1;
			   printf("mtk_nfcho_nfc_event_handle: g_MtkNfchoMainProcessExit = 1\n");
               mtk_nfcho_wifidriver_event_send(&Ret, 1, MTK_WIFINFC_ACT_FROMNFC_SET, NfcStatus);
           }
           break;              
...
int mtk_nfcho_cmd_handler( unsigned char* buf, unsigned int Length, unsigned char Action, unsigned char Type)
{
    ...
    MTK_NFCAPP_LOGD("L,%d,A,%d,T,%d", Length, Action, Type);
            ...
            case MTK_BTNFC_TYPE_MACADDR:
                {
                    ...
                    if (NfcTsetMode == 0)
                    {
                        ...
                        //ReqData.payload[0].data = buf;
                        ReqData.payload[0].data = g_BTPayload;
                        //ReqData.payload[0].lth = Length;
                        ReqData.payload[0].lth = 10;
            ...
            case MTK_WIFINFC_TYPE_NFCSTATUS:
                {                           
                    ...
                    if ((Length == 1) && (buf != NULL))
                    {
                        ...
                        if (( buf[0] < MTK_WIFINFC_SET_END) && (NfcTsetMode == 0))
                        {
                             NfcOpCmd = buf[0];
                            if ( NfcOpCmd == MTK_WIFINFC_SET_OFF)
                            {
                                if ((NfcStatus&MTK_WIFINFC_NFC_ON) == MTK_WIFINFC_NFC_ON)
                                {
                        ...
                        else if (( buf[0] == 30) && (NfcTsetMode == 0)) /////// Test 
                        {
                            if (NfcStatus != 0)
                            {
                        ...
                        else if (( buf[0] == 31) && (NfcTsetMode == 0))/////// Test 
                        {
                            if (NfcStatus != 0)
                            {
                        ...
                        else if (( buf[0] == 32) && (NfcTsetMode == 0))/////// Test Write  Android phone/BD player Tap Data
                        {
                        ...
                        else if ( buf[0] == MTK_WIFINFC_ENABLE_DGB)
                        {
                            if (NfcStatus != 0)
                            {
                                mtk_nfc_app_service_nfcmwdebug(1);
                            }                
                        }
                        else if ( buf[0] == MTK_WIFINFC_DISABLE_DGB)
                        {
                            if (NfcStatus != 0)
                            {
                                mtk_nfc_app_service_nfcmwdebug(0);
                            }
                        }                                              
                        else if ( buf[0] == 255)  // nfc bye bye
                        {
                            mtk_nfc_app_service_exit();
                        }
                        else if ((NfcTsetMode == 1) && (( buf[0] >= MTK_WIFINFC_HWTEST_DEP) && ( buf[0] < MTK_WIFINFC_HWTEST_END)))
                        {
                            MTK_NFC_APP_FUNCTION_POINT AppFun;
                            ...
                            AppFun.send = mtk_nfcho_service_send;
                            AppFun.debug = mtk_nfcho_service_debug;
                             if ( buf[0] == MTK_WIFINFC_HWTEST_DEP)
                             {
                                 TestMode = MTK_NFC_START_HWTEST_DEP;
                             }
                             else if ( buf[0] == MTK_WIFINFC_HWTEST_SWP)
                             {
                                 TestMode = MTK_NFC_START_HWTEST_SWP;
                             }
                             else if ( buf[0] == MTK_WIFINFC_HWTEST_CARD)
                             {
                                 TestMode = MTK_NFC_START_HWTEST_CARD;
                             }
                             else if ( buf[0] == MTK_WIFINFC_HWTEST_VCARD)
                             {
                             ...
                             if (ret == 0)
                             {
                                 ...
                                 if (ret == 0)
                                 {
                                     MTK_NFCAPP_LOGD("Test,%s,OK",g_HwTest[( buf[0]-MTK_WIFINFC_HWTEST_DEP)]);
                                 }
                                 else
                                 {
                                     MTK_NFCAPP_LOGE("Test,%s,NG",g_HwTest[( buf[0]-MTK_WIFINFC_HWTEST_DEP)]);
                                 }
                             }
                             else
                             {
                                 MTK_NFCAPP_LOGE("Enter,%d,Testmode,Fail", buf[0]);
                             } 
                             usleep(2000*1000); // 2000 ms
                             MTK_NFCAPP_LOGD("Nfc killed");
                             NfcStatus = 0;                             
                             g_MtkNfchoServiceThreadExit = 1;
							 printf("mtk_nfcho_cmd_handler: g_MtkNfchoServiceThreadExit = 1\n");
                             g_MtkNfchoMainProcessExit = 1;
							 printf("mtk_nfcho_cmd_handler: g_MtkNfchoMainProcessExit = 1\n");
                        }
                        else
                        {
                           // Command invalid
...
int main (int argc, char **argv)
{
   ...
   strcpy(g_send_if, "rai0");
   strcpy(g_listen_if, "br0");
	...
	printf("main: g_MtkNfchoMainProcessExit = 0\n");
    if(g_bt_pair == 1)
    {
        //service enter
        NfcOpCmd = 1;   // 0: stop poll   1: start poll
        MTK_NFC_APP_FUNCTION_POINT AppFun;
        memset(&AppFun, 0x00, sizeof(MTK_NFC_APP_FUNCTION_POINT));
...
int mtd_read(void)
{
    ...
    {
        g_BTPayload[index--] = (UINT8)mac_addr[i];
    }
    close(fd);
    MTK_NFCAPP_LOGD("\nBluetooth MAC Addr: %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",g_BTPayload[7],g_BTPayload[6],g_BTPayload[5],g_BTPayload[4],g_BTPayload[3],g_BTPayload[2]);
    return 0;
}

int Bluetooth_OOB_write(void)
{
    MTK_NFC_APP_WRITEATA_T ReqData;

    MTK_NFCAPP_LOGD("Bluetooth_OOB_write");
    
    //Get Bluetooth Mac address
	MTK_NFCAPP_LOGD("\n default Bluetooth MAC Addr: %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",g_BTPayload[7],g_BTPayload[6],g_BTPayload[5],g_BTPayload[4],g_BTPayload[3],g_BTPayload[2]);
    mtd_read();
    ...
    mtk_nfc_app_service_write(&ReqData);  
}
