    ...
    function initialFun() {
        ...
        $.ajax({
            ...
            success: function (responseText) {
                ...
                scanEntity = JSON.parse(responseText);//变量被放到了全局环境中(这样写安全不安全？)
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        var ssid = dataObj.ssid;
                        var wifiPassword = dataObj.wifiPassword;
                        var signalStr = dataObj.signal;
                        var securityMeth = dataObj.security;
                        var bssid = dataObj.bssid;//mac地址
                        console.log(bssid);

                        //初始化路由设置界面的路由名称(Chosen是wifi_setting页面提交过来的，包括配置界面的主路由的所有信息和用户输入的正确密码)
                        $("#wifiName>.wifi-l").text(ssid);
                        //初始化路由设置界面的路由密码（wifiPwd是输入的主路由的密码）
                        $("#wifiPwd").val(wifiPassword);
                        //初始化信号强度
                        $("#signalStrength").val(signalStr);
                        //加密方式
                        $("#encrypt").val(securityMeth);
                        //MAC地址
                        $("#mac").val(bssid);
                        console.log($("#mac").val());
                    }
