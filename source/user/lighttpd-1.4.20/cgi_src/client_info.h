...
typedef struct _rt_mac_entry
{
	...
	int  Signal; //设备的信号强度，数值越大，信号越强，得到的总是负值不知为何？
...
//判断nvram的client_info_list是否为空(没有记录过设备信息)
...
/*
  找出mac是否在保存设备信息的文件中，是返回１，否返回其他。
  若果找到，并且参数hostname,和msg_os不为NULL, 则将信息保存到相应的地址
  允许为NULL。
 */
