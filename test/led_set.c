...
int main(int argc, char *argv[])
{
	...
	DBG_MSG("get message from web is %s", input);
#if 0
	char *temp;
	temp = web_get("led_on", input, 0);
	if(strcmp("on", temp) == 0)
	{
		//led on  --> CMCC_LedOff = 0 -->false
		nvram_bufset(RT2860_NVRAM, "CMCC_LedOff", 0);
	...
	{
		//led off --> CMCC_LedOff = 1 -->true
		nvram_bufset(RT2860_NVRAM, "CMCC_LedOff", 1);
	...
	if(strcmp("set", temp) == 0)
	{
		...
		nvram_bufset(RT2860_NVRAM, "CMCC_LedOff", 0);
		nvram_bufset(RT2860_NVRAM, "CMCC_LedBegin", begintime);
		nvram_bufset(RT2860_NVRAM, "CMCC_LedEnd", enditime);
