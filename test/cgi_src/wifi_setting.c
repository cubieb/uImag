...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if(strcmp("commit", web_get("wifiCommit", input, 2)) == 0)
	{
			...
			nvram_bufset(RT2860_NVRAM, "CMCC_Configing", "1");  	//开始配置
	...
	//验证是否连接成功目前暂时不用。date:2017-07-14
	...
	if(strcmp("restart", web_get("init_restart", input, 2)) == 0)
	{
		...
		nvram_bufset(RT2860_NVRAM, "CMCC_Configing", "0");		//配置结束
		nvram_commit(RT2860_NVRAM);
	...
	{
		printf("{\n");
		printf("\"Channel\":\"%s\",", nvram_bufget(RT2860_NVRAM, "Channel"));
		printf("\"ssid\":\"%s\",", nvram_bufget(RT2860_NVRAM, "CMCC_ApCliSsid"));
		printf("\"bssid\":\"%s\",", nvram_bufget(RT2860_NVRAM, "CMCC_SelectApMac"));
		printf("\"security\":\"%s\",", nvram_bufget(RT2860_NVRAM, "CMCC_SelectApSecurity"));
		printf("\"signal\":\"100\",");
		printf("\"wifiPassword\":\"%s\",", nvram_bufget(RT2860_NVRAM, "CMCC_ApCliWPAPSK"));
		...
		if(atoi(nvram_bufget(RT2860_NVRAM, "TxPower")) == 100) 
		{
			printf("\"patterId\":\"throughWall\"");
		} 
		else if(atoi(nvram_bufget(RT2860_NVRAM, "TxPower")) == 95) 
		{
			printf("\"patterId\":\"standard\"");
		} 
		else 
		{
			printf("\"patterId\":\"pregnant\"");
		}
