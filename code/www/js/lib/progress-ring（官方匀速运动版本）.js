...
(function ($) {
    $.fn.loadingRing = function () {
        ...
        //让数字动起来只需要让数字动就行了。
        var percentNum = 0;
        var timer = setInterval(countTimer, 600);
        ...
        function countTimer() {
            ...
            if (percent >= 90 && percentNum >= 90) {
                ...
                // clearInterval(timer);//清除定时器
                console.log("哈哈");
                // // $target.find('.progress-right').stop(true,true);
                // // $target.find('.progress-cover').stop(true,true);
                // $target.find('.progress-left').clearQueue();
